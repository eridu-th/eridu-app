...
router.post('/carry/login', checkHeaders, auth, async (req, res) => {
  ...
  const { carryUsername, carryPassword } = carryUser(
    parseInt(req.user.carry_shop_id),
  );
...
router.post('/carry/order', checkHeaders, auth, async (req, res) => {
  ...
  try {
    if (req.body.token && req.body.data) {
      const response = await callCarry(
        `${carryHost}/api/order`,
        req.body.token,
        req.body.data,
      );
  ...
  if (id) {
    switch (id) {
      ...
      case 257:
        ...
        break;
      case 276:
        carryUsername = process.env.CARRY_ACCOUNT_FT;
        carryPassword = process.env.CARRY_PASSWORD_FT;
        break;
      default:
        break;
    }
